...
export default function Editor({ initialContent = '', onSave, placeholder = 'Commencez à écrire votre histoire...' }: EditorProps) {
  ...
  return (
    <div className="parchment-card rounded-lg overflow-hidden">
      {/* Toolbar */}
      <div className="flex items-center justify-between p-3 border-b border-gold-old/20 bg-dark-void/30">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-1.5 text-sm rounded transition-colors ${
              isPreview 
                ? 'bg-gold-old/20 text-gold-old' 
                : 'text-parchment hover:text-gold-old hover:bg-gold-old/10'
            }`}
          >
      ...
      <div className="min-h-[400px]">
        {isPreview ? (
          <div className="p-6 markdown-content prose prose-invert max-w-none">
      ...
      {/* Footer */}
      <div className="flex items-center justify-between p-3 border-t border-gold-old/20 bg-dark-void/30">
...
function parseMarkdown(text: string): string {
  let html = text
    ...
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/^### (.+)$/gm, '<h3 class="font-gothic text-xl text-gold-old mt-6 mb-3">$1</h3>')
    .replace(/^## (.+)$/gm, '<h2 class="font-gothic text-2xl text-gold-old mt-8 mb-4">$1</h2>')
    .replace(/^# (.+)$/gm, '<h1 class="font-gothic text-3xl text-gold-old mt-10 mb-5">$1</h1>')
    .replace(/\*\*(.+?)\*\*/g, '<strong class="font-semibold text-parchment-light">$1</strong>')
    .replace(/\*(.+?)\*/g, '<em class="italic">$1</em>')
    .replace(/^> (.+)$/gm, '<blockquote class="border-l-3 border-gold-old pl-4 italic text-parchment/80 my-4">$1</blockquote>')
    .replace(/^- (.+)$/gm, '<li class="ml-4">$1</li>')
    .replace(/^(\d+)\. (.+)$/gm, '<li class="ml-4">$2</li>')
    .replace(/`([^`]+)`/g, '<code class="bg-dark-purple/50 px-1.5 py-0.5 rounded text-gold-old font-mono text-sm">$1</code>')
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-gold-old hover:text-gold-bright underline">$1</a>')
    .replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" class="max-w-full rounded-lg my-4" />')
    .replace(/^---$/gm, '<hr class="border-gold-old/20 my-8" />')
